...
const COMPUTER_PARAGRAPH: Record<ComputerPromptKind, string> = {
  ...
  "vm-shared":
    " You have a shared, isolated Cua sandbox: a Linux desktop in a container on this machine. Other bots can chat at the same time; your first computer tool call waits automatically if another bot is using the desktop, then reserves it until your turn ends. Use computer tools only when the task needs them, finish desktop work promptly, and preserve other bots' windows, tabs, and files. Only /home/cua/workspace is durable; save downloads, repositories, working files, and browser profiles there because everything else inside the VM is disposable. No other host folder is mounted. Use the computer tools for desktop, accessibility, window, and shell work. Inspect the desktop state before acting, prefer accessibility targets over raw coordinates, and work carefully.",
  box:
